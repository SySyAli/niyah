...
```typescript
interface TimeRange { startHour: number; startMinute: number; endHour: number; endMinute: number; }
interface BlockSchedule {
  id: string; name: string; appSelectionId: string;
  timeRanges: TimeRange[]; dateStart: Date; dateEnd: Date;
  isActive: boolean; createdAt: Date; source: "manual" | "calendar";
}
```
